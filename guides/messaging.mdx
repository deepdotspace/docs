...
Sends, edits, reactions, and deletes all dispatch optimistically and reconcile through the room. Prefer `softDelete(messageId)` over `remove(messageId)` for user-initiated deletes - it keeps thread replies and read receipts coherent. See [`useChannels`](/sdk-reference/client/messaging#usechannels), [`useMessages` options](/sdk-reference/client/messaging#usemessages-channelid-options), and [`useReactions`](/sdk-reference/client/messaging#usereactions-channelid) for the full API surface.
