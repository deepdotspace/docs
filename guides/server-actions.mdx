...
|---|---|---|
| `tools.create(coll, data)` | `{ recordId, record }` | Create a record. |
| `tools.update(coll, id, patch)` | `{ recordId, record }` | Patch an existing record. |
| `tools.remove(coll, id)` | `{ deleted: true }` | Delete a record. |
| `tools.get(coll, id)` | `{ record }` | Fetch one record. |
| `tools.query(coll, opts?)` | `{ records, count }` | List records with filtering. |
| `tools.integration(endpoint, body)` | `{ response, status? }` | Call a third-party integration. Billing follows `src/integrations.ts`. |

All operations return an `ActionResult<T>` envelope — narrow with `if (result.success)` before accessing `result.data`.
