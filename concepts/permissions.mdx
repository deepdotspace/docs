...
- `teamField` (used by `'team'` / `'access'`) declares which column holds the team ID. The team membership check itself is wired up by the room via [`PermissionContext.isTeamMember`](/sdk-reference/worker/schemas).
