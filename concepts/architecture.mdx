...
**The platform workers** are shared services run by DeepSpace. You never deploy or configure them - they're shared infrastructure. Your worker talks to three of them directly at runtime (auth, API, platform) via the helpers in [Talking to platform workers](#talking-to-platform-workers); the deploy and dispatch workers sit outside your request path.
...
The SDK exposes three fetch helpers in `deepspace/worker` for addressing platform services. Each prefers a Cloudflare service binding when one is configured and falls back to an HTTPS URL otherwise, so the same code path works in production and under `deepspace dev`.
...
**Per-app WebSocket route (`wsRoute`)** - the scaffold ships a `wsRoute` helper defined inline in `worker.ts` (it is not an SDK export - look for the `function wsRoute(...)` declaration near the bottom of the file). It strips `userId`, `userName`, `userEmail`, `userImageUrl`, `role`, and `token` from the URL on every upgrade, then re-applies identity only from a verified JWT. Three states are possible:

- **No token** → anonymous (DO assigns `anon-<uuid>`)
- **Invalid token** → 401
- **Valid token** → identity = JWT `sub` / `name` / `email` / `image`

**Platform worker (cross-app scopes)** - when a `workspace:*`, `dir:*`, or `conv:*` request reaches the platform worker, the same URL parameters are stripped and re-applied from the JWT before the request reaches the shared DO. On HTTP forwards, `x-user-id` is overwritten from the JWT and `x-app-action` is dropped. These scopes require a valid JWT; unauthenticated requests are rejected with 401.
...
<Steps>
  ...
  <Step title="Mount the shared scope in your provider tree">
    ...
    `WORKSPACE_SCHEMAS` lives in `deepspace/worker` because schemas are server-side data — the import is safe in the browser bundle (Vite tree-shakes the worker-only code).
  </Step>
</Steps>
