...
| `allowAnonymous` | `boolean` | Connect without a JWT (default `false`). Required for public pages. See [authentication](/guides/authentication). |
...
<Tabs>
  <Tab title="Create">
    `create` takes the full row shape and returns the server-generated `recordId`. The local store updates immediately; the server confirms milliseconds later.
    ...
    If you need the `recordId` before navigating, use `createConfirmed` to wait for server acknowledgment:
...
|---|---|---|
| `create` | Optimistic | `Promise<string>` (new `recordId`) |
...
`setRole` is admin-only — the Durable Object rejects non-admin callers. See [permissions](/concepts/permissions#roles) for how roles map onto collection RBAC rules.
