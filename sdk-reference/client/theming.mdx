...
```ts
applyDeepSpaceTheme(config: ThemeConfig, root?: HTMLElement): void
clearDeepSpaceTheme(root?: HTMLElement): void
readThemeFromDOM(root?: HTMLElement): ThemeConfig
applyUIThemeTokens(theme: 'light' | 'dark', root?: HTMLElement, accent?: string): void
```
...
| `applyUIThemeTokens` | Sets `UI_TOKENS_LIGHT` / `UI_TOKENS_DARK` for dark/light mode switches. |

`DEEPSPACE_THEME_PROPERTIES` is the list of CSS custom properties the theme defines — useful for introspection.
