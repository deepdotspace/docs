...
function buildCronContext(
  env: {
    ...
    /** Optional in the type, but `ctx.integrations.call(...)` throws at runtime if missing. */
...
- `ctx.integrations.call(endpoint, params)` - proxies through the api-worker as the app owner (signed with `APP_OWNER_JWT`), billed to the app owner. Returns the unwrapped `data` field; throws on non-2xx or `success: false`.
